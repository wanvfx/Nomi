// 从模型 id 猜「图片/视频/文本」类型（Issue #8 中转拉取式接入用）。
//
// 为什么需要：从中转 `/v1/models` 拉到的只有模型 id 字符串，不带类型。要把它们分门别类
// 落进 catalog（image/video/text），得先判断每个 id 是哪类。判断是**启发式**（按关键词），
// 必然有猜错的——所以 UI 给用户一个下拉随手改（onboarding 不写死、judgement 可纠正）。
//
// 单一真相源：关键词表只在这里。新增模型族（如某新视频模型）在对应表加一行即可。

export type GuessableModelKind = "image" | "video" | "text";
...
// 音频/文本判别：嵌入/重排/转写/语音不是对话文本，但本接入面只分三类（image/video/text），
// 这些非生成类暂归 text 桶（用户极少经中转接它们做创作；判错可手改）。
