...
## 第 0 步 · 认清本次的分镜模式（图片 / 视频）

用户消息里会明确说明本次是**图片分镜**还是**视频分镜**——每个 shot 的 `shotKind` 必须与之一致，**整份方案同一种**（用户之后可在编辑器里逐镜改）：
...
- `shotKind`：`"image"` / `"video"`，按第 0 步的本次模式填，整份一致。
- `durationSec`：时长（秒），**仅视频分镜**——默认 5；动作多/节奏慢的镜可给 8。落画布时系统会钳到所选模型上限。图片分镜一律填 0。
