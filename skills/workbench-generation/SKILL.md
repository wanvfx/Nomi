...
2. **顺序叙事的镜头默认按时序连边**：在**同一次 `create_canvas_nodes` 的 `edges` 字段**里把相邻镜头按先后顺序连成线性叙事（n1→n2→n3…，`mode: "reference"`）——节点与边一次提交、用户只确认一次，不要拆成 `connect_canvas_edges` 第二轮（那只用于给画布上已有节点补连）。只有用户明确说"独立镜头 / 不要连线"时才不连。
