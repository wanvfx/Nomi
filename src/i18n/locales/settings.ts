  ...
  automation: {
    ...
    mcp: {
      ...
      customClients: {
        ...
        notInstalled: '待接入',
        detected: '需配置',
        detectedHint: '检测到新连接 · 填写名称、身份 key 和配置路径后接入',
        configure: '重新配置',
        trust: '信任：允许免确认代跑',
        untrust: '未信任：需手动勾选后才允许免确认',
        trustDisabled: '接入后才能信任',
        remove: '移除',
        copyPromptHint: '找不到工具或路径时，复制下面这段贴到你的 AI 工具里，它会帮你完成接入：',
        copyPrompt: '复制接入提示词',
        promptContent: '请帮我接入 Nomi 的 MCP server。\n1. 找到你的 MCP 配置文件（通常是 {{path}}）\n2. 把下面这段配置合并进去（保留你已有的其它 MCP server）：\n\n{{snippet}}\n\n3. 写完后重启工具，确认能看到 nomi_* 工具。\n\n接入完成后，请转告用户：打开 Nomi → 设置 → 自动化与权限 → AI 助手连接 → 自定义客户端，这条连接已带签名身份，点「接入」补上配置路径（或已写配置则跳过），再勾选「信任」即可授权调用完整能力。',
        genericPromptContent: '请帮我接入 Nomi 的 MCP server。\n1. 找到你的 MCP 配置文件（通常是 ~/.yourtool/mcp.json 或 config.toml）\n2. 把下面这段配置写进 mcpServers（JSON）或 [mcp_servers]（TOML）：\n\n{{snippet}}\n\n3. 写完后重启工具，确认能看到 nomi_* 工具。\n\n⚠️ 握手身份：MCP initialize 的 clientInfo.name 报你的产品名（如 WorkBuddy），不要报 server 名 nomi；若工具写死改不了，就直接告知用户。\n\n接入完成后，请转告用户后续操作：\n① 打开 Nomi → 设置 → 自动化与权限 → AI 助手连接 → 自定义客户端\n② 会看到一条「已连通」的新连接，把名字改成你的产品名（如 WorkBuddy）后回车\n③ 点「接入」补上 MCP 配置文件路径，或点「复制」拿带签名的条目贴回工具\n④ 完成后即可「信任」，授权调用完整能力',
        copied: '已复制',
      },
    },
    hosts: {
  ...
  automation: {
    ...
    mcp: {
      ...
      customClients: {
        ...
        detected: 'Needs setup',
        detectedHint: 'New connection detected · fill in name, identity key and config path to connect',
        configure: 'Reconfigure',
        trust: 'Trusted: may run without per-step confirmation',
        untrust: 'Untrusted: must be enabled before it may skip confirmation',
        trustDisabled: 'Connect before you can trust it',
        remove: 'Remove',
        ...
        promptContent: 'Please connect Nomi\'s MCP server for me.\n1. Find your MCP config file (usually {{path}})\n2. Merge this in (keep any other MCP servers you have):\n\n{{snippet}}\n\n3. Restart the tool and confirm you can see nomi_* tools.\n\nAfter connecting, tell the user: open Nomi → Settings → Automation & Permissions → AI assistant connections → Custom clients; this connection already carries its signed identity, click "Connect" to fill in the config path (or skip if already written), then enable "Trust" to authorize full access.',
        genericPromptContent: 'Please connect Nomi\'s MCP server for me.\n1. Find your MCP config file (usually ~/.yourtool/mcp.json or config.toml)\n2. Write this into mcpServers (JSON) or [mcp_servers] (TOML):\n\n{{snippet}}\n\n3. Restart the tool and confirm you can see nomi_* tools.\n\n⚠️ Identity: in the MCP initialize handshake, set clientInfo.name to your product name (e.g. WorkBuddy), not "nomi". If your tool hardcodes this field, tell the user plainly.\n\nAfter connecting, tell the user the next steps:\n① Open Nomi → Settings → Automation & Permissions → AI assistant connections → Custom clients\n② They will see a new "Connected" entry; rename it to your product name (e.g. WorkBuddy) and press Enter\n③ Click "Connect" to fill in the MCP config path, or "Copy" to get the signed entry and paste it back\n④ Then they can "Trust" it to authorize full access',
        copied: 'Copied',
      },
    },
    hosts: {
