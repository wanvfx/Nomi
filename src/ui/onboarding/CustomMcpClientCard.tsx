    ...
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <span
            className="inline-grid h-3.5 w-3.5 cursor-help place-items-center rounded-full border border-nomi-ink-30 text-[9px] font-bold leading-none text-nomi-ink-40"
            aria-label={label}
          >
            ?
          </span>
...
export function CustomMcpClientCard({ info, profiles, trustedHosts, onToggleTrust, onChanged }: CustomMcpClientCardProps): JSX.Element | null {
  ...
  const tc = t('settings.automation.mcp.customClients', { returnObjects: true }) as Record<string, string>
  ...
  const handleSave = () => {
    ...
    const profile = registerBridge({ key: resolvedKey, label: name.trim(), format, configPath: configPath.trim() })
    if (!profile) {
  ...
  const handleCopyPrompt = () => {
    // 真实片段：基础启动条目（未签名 → 接入后按 external 走），对齐样张场景 4。不包 mcpServers
    // 外层键——提示词里已写明「写进 mcpServers」，片段只给 nomi 这一条的值。
    const snippet = JSON.stringify({ nomi: info.server }, null, 2)
    const prompt = t('settings.automation.mcp.customClients.promptContent', { snippet })
            ...
            <div key={profile.key} className="flex items-center gap-2 rounded-nomi-sm border border-nomi-line bg-nomi-paper px-3 py-2">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-1.5 text-body-sm font-medium text-nomi-ink">
                  ...
                  <span className={cn('shrink-0 rounded-full px-1.5 py-0.5 text-micro', trusted ? 'bg-workbench-success-soft text-workbench-success' : 'bg-nomi-ink-05 text-nomi-ink-40')}>
                    {trusted ? tc.trusted : tc.untrusted}
                  </span>
                </div>
                <div className="truncate text-micro text-nomi-ink-40">{profile.configPath}</div>
              </div>
              <button
                type="button"
                title={tc.connect}
