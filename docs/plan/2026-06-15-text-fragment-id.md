...
## 8. 用户拍板（2026-06-15 已定，R3）

1. **同步语义 = 乙2**：单向派生 + 漂移检测 + 显式再同步；不自动覆盖用户手调的下游。
2. **粒度 = 锚/镜头粒度**（一期）：源文本身份绑到方案锚/镜头，不追字符级区间（不碰 Tiptap mark/块锚的脆性；字符级押二期）。
3. **再同步动作 = 两者都给**：同时提供「机械重灌源文本」与「让 AI 重拆这段」，用户选。

→ 实现按 §5 推荐（乙2 + 甲2）+ §7 范围执行；UI（「源已更新」徽标 + 双再同步动作）走 R8 样张后实现。本任务到此（plan + 拍板）为止。
